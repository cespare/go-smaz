// Package trie is an implementation of a trie (prefix tree) data structure
// mapping []bytes to bytes. It provides a small and simple API for usage as a
// set as well as a 'Node' API for walking the trie.
package trie
...
func (t *Trie) Put(k []byte, v byte) bool {
	n := t.root
	for _, c := range k {
		next, ok := n.Walk(c)
		if !ok {
...
func (t *Trie) Get(k []byte) (v byte, ok bool) {
	n := t.root
	for _, c := range k {
		next, ok := n.Walk(c)
		if !ok {
...
// Terminal indicates whether n is terminal in the trie (that is, whether the
// path from the root to n represents an element in the set).
// For instance, if/ the root node is terminal, then []byte{} is in the trie.
func (n *Node) Terminal() bool { return n.terminal }

// Leaf indicates whether n is a leaf node in the trie (that is, whether it has
// children). A leaf node must be terminal (else it would not exist).
// Logically, if n is a leaf node then the []byte represented by the path from
// the root to n is not a proper prefix of any element of the trie.
func (n *Node) Leaf() bool { return !n.hasChildren }

// Val gives the value associated with this node.
// It panics if n is not terminal.
